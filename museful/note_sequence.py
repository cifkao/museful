...
def split_by_instrument(sequence):
    result = []
    instruments = sorted({note.instrument for note in sequence.notes})
    for i in instruments:
        filtered = filter_sequence(sequence, instrument_ids={i}, copy=True)
        set_note_fields(filtered, instrument=0)
        info = [ii for ii in filtered.instrument_infos if ii.instrument == i]
        del filtered.instrument_infos[:]
        filtered.instrument_infos.extend(info)
